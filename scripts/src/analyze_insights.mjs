...
console.log(`- Total Data Output Analyzed: ${bioProjects.totalBioGb?.toFixed(1)} GB`);
console.log(`- Average GB Per Sample: ${bioProjects.avgGbPerSample?.toFixed(2)} GB/sample`);
...
topBioServices.forEach((s, i) => console.log(`  ${i+1}. ${s.name}: ${s.cnt} projects | ${s.samples} samples | ${s.gb?.toFixed(1)} GB`));
...
bioScientists.forEach((s, i) => console.log(`  ${i+1}. ${s.name}: ${s.cnt} bio projects | ${s.samples} samples | ${s.gb?.toFixed(1)} GB`));
...
console.log(`- Wet Lab Data Output: ${wetLabProjects.totalWetLabGb?.toFixed(1)} GB`);
...
console.log(`- Average Lab QC TAT: ${qcStats.avgQcTat?.toFixed(1)} Days`);
...
salesRepPerf.slice(0, 5).forEach((sr, i) => console.log(`  ${i+1}. ${sr.salesPerson} (${sr.territory || 'N/A'}): ₹${(sr.totalRevenue/100000).toFixed(2)} Lakhs | ${sr.projectCount} projects | ${sr.totalSamples} samples`));
